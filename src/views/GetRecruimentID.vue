...
1. **Validate the CV data**: First, check if the provided 'cvData' appears to be a valid CV. A valid CV typically includes information such as:
   ...
   - matchingIds: A list of the top 5 job IDs ranked by suitability. If no jobs match or the data is not a CV, return an empty array.
`
  })
  ...
  try {
    ...
    if (parsedData.isValidCV) {
      matchingRecruitmentIds.value = parsedData.matchingIds || []
